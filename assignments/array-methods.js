...
let fullName = [];
let nameArr = runners.forEach(function firstLast(item) {
...
let sortLastName = runners.map((item) => {
    return item.last_name;
});

console.log(sortLastName.sort())
// Problem 2
//Find companies that start with "S"

let findCompName = runners.filter((item) => item.company_name[0].toUpperCase(0) === "S")
console.log(findCompName);

// Problem 3
//Add $100 to each donation muahahahahaha!!!

newDonation = [];

let addDonation = runners.forEach((item) => {
    newDonation.push(item.donation + 100)
});
